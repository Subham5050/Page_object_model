...
@BeforeClass
	...
	public void MD_Indexing_TC02() throws InvalidFormatException, IOException, Exception
	{
	   ...
	   Browser.test = Browser.report.startTest(value);
		...
		Thread.sleep(2000);
		homepage.search_TV(elib.getExceldata("DataSheet.xlsx", "Sheet1", 0, 1));
		Browser.test.log(LogStatus.PASS, "Entered TV and clicked search");
		log1.info("Entered TV and clicked search");
		
		Boolean b=search.validate_searchlist_television();
	 
		if(b==true)
		{
			 log1.info(value+" test case passed successfully");
			 Browser.test.log(LogStatus.PASS, value +" Passed Successfully"+Browser.test.addScreenCapture(capture(driver)));
		}
		else
		{
			 Browser.test.log(LogStatus.FAIL, value +" Failed");
